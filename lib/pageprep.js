...
(function (root) {
  ...
  // The coarse-page pass at 2x reaches a little further, because what it is
  // for is a page that has few pixels for what is on it — and a slide rendered
  // at 144 dots to the inch is 1920 across, over the line above while being
  // exactly the kind of page that needs it. Past this the page has pixels to
  // spare and doubling it is four times the area for nothing.
  const UPSCALE_MAX_LONG_EDGE = 2400;
  ...
  // Dark bands that still hold bright ink — white / light type on a coloured
  // header bar. Normal OCR is tuned for dark-on-light and often returns
  // nothing there (measured: "Middle East" on a red Charles & Keith header).
  //
  // Kept deliberately narrow: wide, short bands with a dark fill and a bright
  // minority. Solid shadows, photos and light body text stay out. Cap the
  // count and area so a second OCR pass cannot dominate the page cost.
  const DARK_CELL = 24;
  const DARK_MAX_REGIONS = 6;
  const DARK_MAX_AREA_FRAC = 0.06;

  function darkInkRegions(gray, width, height) {
    const cell = DARK_CELL;
    const cols = Math.max(1, Math.ceil(width / cell));
    const rows = Math.max(1, Math.ceil(height / cell));
    const hits = [];
    for (let cy = 0; cy < rows; cy++) {
      for (let cx = 0; cx < cols; cx++) {
        ...
        let sum = 0;
        let n = 0;
        let glyph = 0; // light ink that is not blank paper
        let dark = 0;
        let paper = 0;
        for (let y = y0; y < y1; y++) {
          const row = y * width;
          for (let x = x0; x < x1; x++) {
            ...
            n++;
            if (v < 90) dark++;
            else if (v >= 160 && v <= 235) glyph++;
            else if (v > 240) paper++;
          }
        }
        const mean = sum / Math.max(1, n);
        const gl = glyph / Math.max(1, n);
        const dk = dark / Math.max(1, n);
        const pp = paper / Math.max(1, n);
        // Dark fill + light glyphs. Paper at the edge of a solid shadow must
        // not count as "bright text" (that was inventing false header bands).
        if (mean < 115 && gl >= 0.05 && gl <= 0.5 && dk >= 0.3 && pp < 0.35) {
          hits.push({ cx: cx, cy: cy });
        }
      }
    }
    ...
    for (let i = 0; i < hits.length; i++) {
      for (let j = i + 1; j < hits.length; j++) {
        if (Math.abs(hits[i].cx - hits[j].cx) <= 1
          && Math.abs(hits[i].cy - hits[j].cy) <= 1) unite(i, j);
      }
    }
    ...
    for (const cells of groups.values()) {
      ...
      const r = Math.min(width, maxX + pad);
      const b = Math.min(height, maxY + pad);
      const w = Math.max(1, r - x);
      const h = Math.max(1, b - y);
      const area = w * h;
      const ratio = w / Math.max(1, h);
      // Header-like: wider than tall, not a tiny chip, not a huge slab.
      if (w < 72 || h < 14 || h > 80) continue;
      if (ratio < 2.2) continue;
      if (area > pageArea * DARK_MAX_AREA_FRAC) continue;
      if (area < 72 * 16) continue;
      regions.push({ x: x, y: y, w: w, h: h, area: area });
    }
    regions.sort((a, b) => b.area - a.area);
    return regions.slice(0, DARK_MAX_REGIONS);
  }

  // Negate a prepared-canvas crop so white-on-dark becomes dark-on-light.
  function invertRegionCrop(canvas, region) {
    const c = typeof OffscreenCanvas !== 'undefined'
      ? new OffscreenCanvas(region.w, region.h)
      : document.createElement('canvas');
    c.width = region.w;
    c.height = region.h;
    const ctx = c.getContext('2d', { alpha: false });
    ctx.drawImage(canvas, region.x, region.y, region.w, region.h, 0, 0, region.w, region.h);
    const image = ctx.getImageData(0, 0, region.w, region.h);
    const data = image.data;
    ...
    ctx.putImageData(image, 0, 0);
    return { canvas: c, x: region.x, y: region.y, w: region.w, h: region.h, inverted: true };
  }
  ...
  function pageAssets(gray, width, height) {
    ...
    const assets = {
      ...
      allowUpscale: longEdge(width, height) <= UPSCALE_MAX_LONG_EDGE,
    };
  ...
  root.BlindedPagePrep = {
    WORK_LONG_EDGE,
    SMALL_TEXT_MAX_LONG_EDGE, UPSCALE_MAX_LONG_EDGE,
    workSize,
    downsampleGray,
    buildPyramid,
    pyramidLevel,
