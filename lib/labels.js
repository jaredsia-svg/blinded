...
(function (root) {
  ...
  //
  // Card, bank-account and IP categories were removed along with the detectors
  // that produced them. A draft saved before that carries labels for them;
  // categoryFor falls back to 'manual', so an old draft reopens with a
  // readable label rather than an error.
  const CATEGORIES = {
