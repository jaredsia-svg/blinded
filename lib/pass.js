...
(function (root) {
  ...
  let publicJwk = null;
  // A different key forgets the one imported for the last: otherwise every
  // check after a change would still be made against the old key, and a pass
  // signed with the new one would read as forged.
  function useKey(jwk) {
    const next = jwk || null;
    if (JSON.stringify(next) !== JSON.stringify(publicJwk)) imported = null;
    publicJwk = next;
  }
  ...
  // which is why the buying page shows the pass itself and says to keep a
  // copy, and why the receipt's transaction id can fetch it back.
