...
(function (root) {
  ...
  // Content words of a typed phrase (OCR + Comprehensive). Drop connectors
  ...
  function findOcrPhraseByParts(text, term) {
    ...
    const pools = parts.map(part => findTerms(text, [part], { fromOcr: true, _skipPhraseParts: true }));
  ...
  // Should a Comprehensive shape hit for `term` be refused because it sits
  ...
  root.BlindedDetect = {
    findAll, findTerms, termPattern, phraseContentParts, findOcrPhraseByParts, hostContradictsShapeTerm, lettersOf, applyToText, resolveOverlaps,
    DETECTORS,
