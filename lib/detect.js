...
(function (root) {
  ...
  // The same word under the reader's commonest confusions, for seeding only.
  //
  // Some misreads are not a letter wrong but a shape read as its look-alike:
  // measured on a photographed slide, the reader read the heading "F&N's" as
  // "FaN's". That is zero edits away once "&" and "a" count as one mark, and
  // hopelessly far under lettersOf, which drops the "&" and leaves two
  // letters -- too short for the one-edit rule to be allowed near it.
  //
  // So both sides are folded -- each look-alike class to one letter, case and
  // a trailing possessive dropped -- and must then be equal. Not near: equal,
  // and at least three marks long. It only ever says where the shape check
  // should look; whatever is found there still has to score as a picture of
  // the word, so a confusion alone never marks anything.
  const OCR_LOOKALIKES = [
    [/rn/g, 'm'], [/vv/g, 'w'], [/cl/g, 'd'],
    [/[0oq]/g, 'o'], [/[1il|!]/g, 'l'], [/[5s$]/g, 's'], [/[8b]/g, 'b'],
    [/[&a]/g, 'a'], [/[2z]/g, 'z'], [/[6g]/g, 'g'],
  ];
  function ocrFold(text) {
    let s = String(text || '').toLowerCase()
      ...
      .replace(/'s$/, '')
      .replace(/[^a-z0-9&$|!]+/g, '');
    for (const [from, to] of OCR_LOOKALIKES) s = s.replace(from, to);
    return s;
  }
  function ocrConfusableMatch(token, part) {
    const a = ocrFold(token);
    const b = ocrFold(part);
    return b.length >= 3 && a === b;
  }
  ...
  function hostContradictsShapeTerm(hostText, term) {
    ...
    // A host that is the term under the reader's look-alikes ("FaN's" for
    // "F&N") is the reader agreeing, badly spelt, not a different word.
    if (ocrConfusableMatch(host, needle)) return false;
    // A host that is one word of the phrase is the reader agreeing with part
    // of it, not reading something else there: a confident "Inderpreet" under
    // a match for "Inderpreet Wadhwa" refused the match, because the host was
    // not the whole phrase.
    // The same for its connectors: a confident "and" under "Fraser and Neave".
    const words = needle.split(/\s+/).filter(Boolean);
    if (words.length >= 2 && words.some(part => findTerms(host, [part]).length > 0
      || ocrConfusableMatch(host, part)
      || lettersOf(host).toLowerCase() === lettersOf(part).toLowerCase())) return false;
  ...
  root.BlindedDetect = {
    findAll, findTerms, termPattern, phraseContentParts, findOcrPhraseByParts, ocrFuzzyPartMatch, ocrConfusableMatch, ocrFold, levenshtein, hostContradictsShapeTerm, lettersOf, applyToText, resolveOverlaps,
    DETECTORS,
