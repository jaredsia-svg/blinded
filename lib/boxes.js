...
(function (root) {
  ...
  // A rectangle after its page has been turned a quarter clockwise.
  //
  // `pageHeight` is the page's height before the turn, which is the turned
  // page's width. The corner that ends up at the top left is not the one that
  // started there: turning clockwise sends the bottom-left corner to the top
  // left, so the new y is the old x, and the new x is measured from the old
  // bottom edge rather than the old top.
  function turn(rect, pageHeight) {
    return {
      ...rect,
  ...
  root.BlindedBoxes = {
    ...
    mergeRects, padRects, sameLine, rectAt, rectFromDrag, turn,
