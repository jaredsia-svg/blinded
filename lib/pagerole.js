...
(function (root) {
  ...
  // Comprehensive shape templates for ≤3-letter acronyms false-match inside
  // longer logo wordmarks (KAS⊂TEXAS). OCR/text often never names the host on
  // a logo collage, so the reader veto cannot fire — refuse those short shape
  // hits when they sit in a logo_grid region. Real short acronyms elsewhere
  // (body text) are unchanged; use Images to pick a logo glyph if needed.
  function allowShortAcronymShape(roles, term, rect) {
    const letters = String(term || '').replace(/[^A-Za-z]/g, '');
    if (letters.length === 0 || letters.length > 3) return true;
    if (!roles) return true;
    const hit = roleAt(roles, rect);
    if (hit && hit.role === 'logo_grid') return false;
    // Logo-collage pages often lack per-cluster OCR; pageHint still catches them.
    if (roles.pageHint === 'logo_grid') return false;
    return true;
  }
