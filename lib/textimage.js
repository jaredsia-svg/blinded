...
(function (root) {
  ...
  const SWEEP_FACES = FACES.filter(f =>
    f.family.indexOf('Helvetica') === 0 && f.weight === '700');


  const PHRASE_STOP = new Set(['and', 'or', 'of', 'the', 'a', 'an', '&', '+']);

  // Content words of a typed phrase for Comprehensive per-word sweeps.
  ...
  root.BlindedTextImage = {
    renderTerm, templatesFor, fontFor, shapeRelief, phraseContentParts, FACES, FALLBACK_FACES,
