...
(function (root) {
  ...
  // A picked mark shrunk to working size: every output pixel is the average
  // of the source pixels it covers, not four of them. Bilinear sampling reads a 2x2
  // neighbourhood whatever the ratio, so at a third of the size it skips two
  // pixels in three: a thin stroke lands on a sample or between two, and the
  // shrunken mark is a different shape from the same mark drawn small, which
  // is what the page holds. Measured on an investor deck: its cover wordmark,
  // picked, scored 0.68 against the identical wordmark in every slide's
  // footer at 0.45 the size -- under the bar, so twenty-three copies went
  // unfound.
  function shrink(src, sw, sh, dw, dh) {
    const out = new Float32Array(dw * dh);
    const xr = sw / dw;
    const yr = sh / dh;
    // Per output column, the source columns it covers and how much of each.
    const cols = [];
    for (let x = 0; x < dw; x++) {
      const a = x * xr, b = a + xr;
      const from = Math.floor(a), to = Math.min(sw, Math.ceil(b));
      const w = [];
      for (let i = from; i < to; i++) w.push(Math.min(b, i + 1) - Math.max(a, i));
      cols.push({ from, w });
    }
    const row = new Float32Array(dw);
    for (let y = 0; y < dh; y++) {
      const a = y * yr, b = a + yr;
      const from = Math.floor(a), to = Math.min(sh, Math.ceil(b));
      row.fill(0);
      let total = 0;
      for (let j = from; j < to; j++) {
        const wy = Math.min(b, j + 1) - Math.max(a, j);
        total += wy;
        const line = j * sw;
        for (let x = 0; x < dw; x++) {
          const c = cols[x];
          let sum = 0;
          for (let k = 0; k < c.w.length; k++) sum += src[line + c.from + k] * c.w[k];
          row[x] += sum * wy;
        }
      }
      for (let x = 0; x < dw; x++) out[y * dw + x] = row[x] / (total * xr);
    }
    return out;
  }
  ...
  // deck looking for one word: ninety-four percent of the second check
  ...
  // ---------- the same transforms, in WebAssembly ----------
  //
  // lib/fft.c: the forward transform, the multiply and the inverse, four
  // lanes at a time -- the part of the search the time goes on. Loaded where
  // the search runs (each worker has its own), and used when it loaded;
  // everything else, and everything when it did not, stays on the loop
  // below. It computes the same planes: the self-test holds the two paths
  // together on real inputs.
  let kernel = null;

  function useKernel(instance) {
    kernel = instance && instance.exports && instance.exports.fft2
      ? { x: instance.exports, stages: new Map() } : null;
    return Boolean(kernel);
  }

  // Refused by an old browser or a policy without 'wasm-unsafe-eval', or not
  // found at all: the search runs exactly as it did without it.
  async function loadKernel(url) {
    try {
      const answer = await fetch(url, { credentials: 'omit' });
      if (!answer.ok) return useKernel(null);
      const made = await WebAssembly.instantiate(await answer.arrayBuffer());
      return useKernel(made.instance);
    } catch {
      return useKernel(null);
    }
  }

  // Twiddles laid out stage by stage, the order fft.c reads them in.
  function stagesFor(n) {
    let at = kernel.stages.get(n);
    if (at) return at;
    const turn = twiddlesFor(n);
    const cos = new Float32Array(Math.max(1, n - 1));
    const sin = new Float32Array(Math.max(1, n - 1));
    let off = 0;
    for (let len = 2; len <= n; len <<= 1) {
      const half = len >> 1;
      const step = n / len;
      for (let k = 0; k < half; k++) {
        cos[off + k] = turn.cos[k * step];
        sin[off + k] = turn.sin[k * step];
      }
      off += half;
    }
    at = { cos, sin };
    kernel.stages.set(n, at);
    return at;
  }

  // The buffers for one plane's tiles, laid out in the kernel's memory above
  // its own data, the memory grown when a larger tile needs it. Views are
  // made afresh each time: growing the memory detaches every earlier one.
  function kernelSpace(tileW, tileH) {
    const x = kernel.x;
    const n = tileW * tileH;
    const align = v => (v + 15) & ~15;
    let at = align(x.__heap_base.value);
    const lay = count => { const here = at; at = align(at + count * 4); return here; };
    const place = {
      re: lay(n), im: lay(n), sr: lay(n), si: lay(n),
      wr: lay(tileW), wi: lay(tileW), hr: lay(tileH), hi: lay(tileH),
    };
    const have = x.memory.buffer.byteLength;
    if (at > have) x.memory.grow(Math.ceil((at - have) / 65536));
    const view = (ptr, count) => new Float32Array(x.memory.buffer, ptr, count);
    const rows = stagesFor(tileW);
    const cols = stagesFor(tileH);
    view(place.wr, rows.cos.length).set(rows.cos);
    view(place.wi, rows.sin.length).set(rows.sin);
    view(place.hr, cols.cos.length).set(cols.cos);
    view(place.hi, cols.sin.length).set(cols.sin);
    return {
      place,
      re: view(place.re, n), im: view(place.im, n),
      sr: view(place.sr, n), si: view(place.si, n),
      forward() { x.fft2(place.re, place.im, tileW, tileH, 0, place.wr, place.wi, place.hr, place.hi); },
      multiply() { x.multiply(place.re, place.im, place.sr, place.si, n); },
      inverse() { x.fft2(place.re, place.im, tileW, tileH, 1, place.wr, place.wi, place.hr, place.hi); },
    };
  }
  ...
  function numeratorPlane(gray, width, height, template) {
    ...
    // In the kernel's memory when there is a kernel, so the transforms work
    // on the tiles where they lie rather than on copies of them.
    const K = kernel ? kernelSpace(tileW, tileH) : null;
    if (K) { K.sr.set(sr); K.si.set(si); }
    const re = K ? K.re : new Float32Array(n);
    const im = K ? K.im : new Float32Array(n);
  ...
  // Every position's sum of products for a window too small to transform,
  // in the kernel: the loop below, sixteen positions at a time. Measured on a
  // two-page deck once the transforms were in the kernel, this loop was
  // three quarters of the second check's arithmetic -- refinement and
  // verification score thousands of small windows, each position a full pass
  // over the template. Null where there is no kernel, and the loop runs.
  function directPlane(gray, width, height, template) {
    if (!kernel || !kernel.x.dots) return null;
    const x = kernel.x;
    const tw = template.width;
    const th = template.height;
    const cols = width - tw + 1;
    const rows = height - th + 1;
    if (cols <= 0 || rows <= 0) return null;
    const align = v => (v + 15) & ~15;
    const g = align(x.__heap_base.value);
    const t = align(g + width * height * 4);
    const o = align(t + tw * th * 4);
    const end = o + cols * rows * 8;
    const have = x.memory.buffer.byteLength;
    if (end > have) x.memory.grow(Math.ceil((end - have) / 65536));
    new Float32Array(x.memory.buffer, g, width * height).set(gray);
    new Float32Array(x.memory.buffer, t, tw * th).set(template.zero);
    x.dots(g, width, t, tw, th, o, cols, rows);
    const out = new Float64Array(x.memory.buffer, o, cols * rows).slice();
    // The kernel sums each patch relative to its first pixel (see fft.c),
    // which is exact only if the template sums to zero, and in single
    // precision it sums to nearly zero. Adding that pixel times the
    // template's actual sum back makes it exact whatever the sum is: the
    // same number the loop makes, not one that is merely as good.
    const sumT = templateSum(template);
    if (sumT !== 0) {
      for (let y = 0; y < rows; y++) {
        for (let xx = 0, at = y * cols, from = y * width; xx < cols; xx++) {
          out[at + xx] += gray[from + xx] * sumT;
        }
      }
    }
    return out;
  }
  const templateSums = new WeakMap();
  function templateSum(template) {
    let sum = templateSums.get(template);
    if (sum === undefined) {
      sum = 0;
      for (let i = 0; i < template.zero.length; i++) sum += template.zero[i];
      templateSums.set(template, sum);
    }
    return sum;
  }
  ...
  function correlate(gray, width, height, template, options) {
    ...
    const plane = (planeW * (toY + 1)) >= FFT_MIN_POSITIONS
      ? numeratorPlane(gray, width, height, template)
      : directPlane(gray, width, height, template);
  ...
  root.BlindedMatch = {
    ...
    numeratorPlane, directPlane, shrink, dotAt, FFT_MIN_POSITIONS, useKernel, loadKernel,
    kernelLoaded: () => Boolean(kernel),
  };
