...
(function (root) {
  ...
  const SCALES = (() => {
    ...
    for (let s = out[0] / far; s >= 0.24; s /= far) out.unshift(Math.round(s * 1000) / 1000);
    for (let s = near; s <= NEAR_HIGH; s *= near) out.push(Math.round(s * 1000) / 1000);
    for (let s = out[out.length - 1] * far; s <= 4.001; s *= far) {
  ...
  // And a ceiling, so a page of near-misses cannot make a search crawl.
  //
  // It was 16, which is fine for a page carrying a logo two or three times
  // and quietly wrong for the pages this tool is actually pointed at. On one
  // page of a real CIM the same badge appears 51 times — 9 of them the exact
  // mark that was picked, the rest its siblings with a different letter in
  // the middle. The 16 best refined candidates were all copies sitting on
  // white, and the one copy sitting on a purple hexagon refined a little
  // lower, fell off the end of the shortlist, and was never verified at all.
  // Scored properly it comes back at 0.93.
  const VERIFY_LIMIT = 64;

  // How far below the best a candidate may score and still be worth
  // verifying. Past this it is a different thing that happens to resemble the
  // mark, and verifying a page full of those is what the ceiling is for.
  // Together they say: always look at the strongest handful, and look at
  // everything that is nearly as good as the best thing on the page.
  const VERIFY_NEAR = 0.2;
  const VERIFY_ALWAYS = 16;
  ...
  //
  // Raised from 110 with the ladder's rungs going from 13 to 20 and with the
  // 51-badge page above in mind: a page dense with near-identical marks fills
  // a budget of 110 with the easy copies, and the interesting one — the copy
  // that sits on a different background — is what falls off the end.
  ...
  root.BlindedMatch = {
    ...
    COARSE_THRESHOLD, MAX_CANDIDATES, REFINE_STEPS, VERIFY_NEAR, VERIFY_ALWAYS,
  };
