...
(function (root) {
    ...
    for (let i = 0; i < first.length; i++) {
      ...
      for (let p = 1; p < parts.length; p++) {
        ...
        for (let j = 0; j < pools[p].length; j++) {
          ...
          if (!phraseHitsAdjacent(prev, cand, (skippedBetween && skippedBetween[p - 1]) || 0)) continue;
          const gap = cand.x - (prev.x + prev.w);
  ...
  root.BlindedMatch = {
    ...
    phraseHitsAdjacent, unionHitRect, pairPhraseHits, skippedConnectorsBetween,
