...
(function (root) {
  ...
  function nominate(gray, pageW, pageH, coarse, scales, perScale) {
    // How many nominations each scale contributes. A page with one copy of a
    // logo needs very few; a page with fifty near-identical badges needs
    // enough that the odd one out is still among them.
    const keep = perScale || 16;
    const found = [];
  ...
  function searchPage(gray, pageW, pageH, ready, opts) {
    ...
    // The strongest handful always, plus everything that is nearly as good as
    // the best thing on the page, up to the ceiling.
    //
    // A flat cap loses the copy that is different in some way that costs it a
    // little at refinement — sitting on a patterned background, say — because
    // the identical copies rank above it and there are more of them than the
    // cap. On a page with 51 of them that is exactly what happened, and the
    // one that was missed scored 0.93 once anything looked at it.
    const top = ranked.length ? ranked[0].hit.score : 0;
    const band = top - Match.VERIFY_NEAR;
    const shortlist = ranked
      .filter((item, i) => i < Match.VERIFY_ALWAYS || item.hit.score >= band)
