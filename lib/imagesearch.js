...
(function (root) {
  ...
  function prepareAt(template, size) {
    ...
    // Shrunk by averaging where it is shrunk a long way (see Match.shrink):
    // the page's copies were drawn small, and a pick sampled down to their
    // size has to look like something drawn small too. The page is resized
    // as it always was.
    // A word the check drew itself is left as it was measured: its bars were
    // set against the old sampling, and it was never a copy of anything.
    // And not for the coarse pass, whose scores rank a shortlist: they were
    // measured with the old sampling, and changing them lost a footer copy
    // from the shortlist on a busy page. The finer stages decide the score.
    const far = !template.face && size.target >= Match.FINE_SIZE.target
      && (template.width > w * 1.5 || template.height > h * 1.5);
    const small = far && Match.shrink
      ? Match.shrink(template.gray, template.width, template.height, w, h)
      : Match.resize(template.gray, template.width, template.height, w, h);
  ...
  function nominate(gray, pageW, pageH, coarse, scales, perScale, pyramid, coarseBar) {
    const keep = perScale || 8;
    coarseBar = coarseBar || Match.COARSE_THRESHOLD;
    const found = [];
    ...
    for (const scale of scales) {
      ...
      // Prefer a precomputed pyramid level close to the target size so a
      // multi-template sweep does not resample the full working page from
      // scratch on every scale.
      let scaled;
      if (pyramid && pyramid.length) {
        const level = PagePrep.pyramidLevel(pyramid, pw);
        scaled = (level.width === pw && level.height === ph)
          ? level.gray
          : Match.resize(level.gray, level.width, level.height, pw, ph);
      } else {
        scaled = Match.resize(gray, pageW, pageH, pw, ph);
      }
      const hits = Match.correlate(scaled, pw, ph, coarse.prepared,
        { threshold: coarseBar, anyPolarity: true });
  ...
  // Under this many pixels on its short side, a copy is checked at a few
  // sizes either side of refinement's (see searchPage).
  const SMALL_COPY = 40;
  const SMALL_COPY_AREA = 1500;
  ...
  // And how far when the page itself is coarse rather than the lettering
  // small.
  //
  // A page rendered at 96 dots to the inch holds two thirds of the pixels of
  // one at 150, and a photographed slide holds whatever the camera gave it.
  // Correlation is not scale-free in practice: below a certain number of
  // pixels the strokes that tell one mark from another have been averaged
  // away before the matcher ever sees them, and no threshold recovers them.
  // Resampling up does not invent detail, but it does give the correlator
  // enough samples to land on the peak instead of stepping over it — measured
  // on a page where resampling by half scored 0.677 against a genuine 3x
  // re-render's 0.655, which is the same answer for none of the cost.
  //
  // Two is the cap. A third multiple costs four times the area for detail
  // that is not there, which in a browser tab is a search nobody waits for.
  const COARSE_PAGE_UPSCALE = 2;

  // Under this many dots to the inch a page counts as coarse. 150 is where
  // ordinary body type stops having enough pixels per stroke: at 96 dpi a
  // 9-point word is about eleven pixels tall, and its counters close up.
  const COARSE_DPI = 150;

  // And a mark this small in its own right is looked for the same way, however
  // fine the page it was cut from: the shortest side of the template is what
  // limits how much of its shape survives the working resample.
  const TINY_TEMPLATE_SHORT = 12;

  // The dots per inch a page was rendered at, or null when it cannot be known
  // — an image file has pixels and no paper size to measure them against.
  function dpiOf(page) {
    // A photograph's "points" are its own pixels — there is no paper behind
    // it — so measuring dots per inch off one says 72 about every image ever
    // opened, which is a number about nothing.
    if (!page || page.fromImage) return null;
    const pt = page && page.widthPt;
    const px = page && page.source && page.source.width;
    if (!(pt > 0) || !(px > 0)) return null;
    return (px / pt) * 72;
  }

  // How far up this page should be resampled for a second look, or 1 for none.
  //
  // Ordered by how much they need it. A coarse page needs the most, because
  // everything on it is short of pixels; a small mark on a fine page needs the
  // same treatment for its own sake; small lettering on an ordinary page needs
  // the gentler pass that was already here.
  function upscaleFor(ready, assets, opts) {
    const template = ready.template;
    const short = template ? Math.min(template.width, template.height) : 0;
    const most = COARSE_PAGE_UPSCALE;
    if (assets.allowUpscale) {
      const dpi = opts && opts.pageDpi;
      if (dpi && dpi < COARSE_DPI) return most;
      if (short && short < TINY_TEMPLATE_SHORT) return most;
    }
    // A big page has pixels to spare, and the gentler pass costs less.
    if (!assets.allowSmallText) return 1;
    return ready.smallText ? SMALL_TEXT_UPSCALE : 1;
  }
  ...
  function upscaledOf(gray, width, height, factor) {
    const by = factor || SMALL_TEXT_UPSCALE;
    let byFactor = upscaled.get(gray);
    if (!byFactor) { byFactor = new Map(); upscaled.set(gray, byFactor); }
    const held = byFactor.get(by);
    if (held) return held;
    const w = Math.round(width * by);
    const h = Math.round(height * by);
    const up = { gray: Match.resize(gray, width, height, w, h), width: w, height: h };
    byFactor.set(by, up);
    return up;
  }

  function searchPage(gray, pageW, pageH, ready, opts) {
    // The rungs are chosen for this template's size rather than shared by
    // every template: what decides whether a copy can be told apart from the
    // page is how many pixels it has, and that is the template's size times
    // the scale. See MIN_COPY in match.js for the two failures that measured.
    const scales = opts.scales
      || Match.scalesFor(ready.template.width, ready.template.height);
    ...
    // Cap the correlator's working resolution, build a grey pyramid once, and
    // mask to text-like regions. Verify still uses a crop of the original
    // pixels so scores stay honest on a 12MP photo without paying 12MP for
    // every coarse scale.
    const assets = PagePrep.pageAssets(gray, pageW, pageH);
    const work = assets.work;
    const workGray = assets.masked;
    const workW = work.width;
    const workH = work.height;
    const toOrig = work.scale === 1 ? 1 : (1 / work.scale);

    // A seeded pass looks only where it was pointed. Nomination is the
    // expensive half — every scale, over the whole page — and the whole point
    // of a seed is that something else has already done that job better. With
    // it left in, a pass that checks three places cost as much as one that
    // checks the document.
    const nominated = opts.seedsOnly && opts.seeds && opts.seeds.length ? []
      : interleave(
        nominate(workGray, workW, workH, coarse, scales, opts.perScale, assets.pyramid,
          opts.coarseThreshold),
        opts.maxCandidates || Match.MAX_CANDIDATES);

    // Places the caller already knows about.
    //
    // Nomination correlates a shrunken page and proposes where the shape might
    // be. It is the half that fails on a page it cannot see into: lettering in
    // artwork, a word whose neighbours the page reader misread, a copy that
    // simply falls outside a shortlist a hundred long. The reader's own word
    // boxes are a second opinion about *where* — and a better one, because
    // even when it reads a word wrongly the box it drew is in the right place
    // and the right size, which also hands over the scale for free.
    //
    // They are appended after the shortlist has been cut, and they carry a
    // flag that exempts them from the verify gate and the verify limit below,
    // so a seed can never displace a nomination. What a seed does not get is
    // any relief from the threshold: it says where to look, and the same bar
    // decides what is found there.
    // A few sizes per seed, not one.
    //
    // The box a reader draws is round the ink it saw, and that is not the box
    // this template would occupy: it takes in a comma, or a tall neighbour, or
    // clips a descender. Measured on the photographed slide, the box round
    // "Middle" is two and a half times the height of the word, and a single
    // scale read off its width refined to 0.515 where the right size scores
    // 0.636. Width and height each give an estimate; the rungs span both and
    // a third either side, which is a few dozen refinements in total against
    // a nomination pass that is the whole page at every scale.
    const SEED_SPREAD = 1.35;
    const seeded = [];
    for (const seed of opts.seeds || []) {
      const w = seed.w * work.scale;
      const h = seed.h * work.scale;
      const byWidth = template.width > 0 ? w / template.width : 0;
      const byHeight = template.height > 0 ? h / template.height : 0;
      const guesses = [byWidth, byHeight].filter(one => one > 0 && isFinite(one));
      if (!guesses.length) continue;
      const low = Math.min(...guesses) / SEED_SPREAD;
      const high = Math.max(...guesses) * SEED_SPREAD;
      // ladder is anchored on the template's own size, which is what `scale`
      // means here; a range that misses every rung still gets its own guesses.
      const rungs = Match.ladder(low, high);
      for (const scale of rungs.length ? rungs : guesses) {
        seeded.push({
          x: seed.x * work.scale, y: seed.y * work.scale, w, h,
          scale, score: 1, seeded: true,
        });
      }
    }

    // Locate everything first on the working grey. This is the cheap half.
    const located = [];
    for (const candidate of nominated.concat(seeded)) {
      const hit = refine(workGray, workW, workH, template, fine, candidate);
      if (hit) located.push({ hit, scale: candidate.scale, seeded: candidate.seeded });
    }

    // Then verify only the plausible ones, best first — against a crop of the
    // *original* page, with coordinates mapped back from working space.
    const gate = opts.verifyGate === undefined ? Match.VERIFY_GATE : opts.verifyGate;
    const shortlist = located
      .filter(item => item.hit.score >= gate && !item.seeded)
      .sort((a, b) => b.hit.score - a.hit.score)
      .slice(0, opts.verifyLimit || Match.VERIFY_LIMIT);
    // A seed is verified whatever it refined to. The gate and the limit are
    // both ways of spending a budget on the most promising guesses, and a seed
    // is not a guess — something else has already said the word is here.
    for (const item of located) if (item.seeded) shortlist.push(item);
    ...
    for (const item of shortlist) {
      ...
      const origHit = {
        ...
        h: workHit.h * toOrig,
        score: workHit.score,
        scale: workHit.scale * toOrig,
        inverted: workHit.inverted,
      };
      // Refinement's size, and for a small copy its near neighbours. A mark drawn small is
      // unforgiving about size: measured on an investor deck, one footer
      // wordmark on every slide verified at 0.914 where refinement landed at
      // 175 pixels wide and at 0.699 where it landed at 178 -- under half a
      // pixel of stroke, and twenty copies of one mark split across the bar.
      // A step and a half either way costs four small windows.
      // Only for a copy drawn small, where half a pixel is a stroke.
      // And only for a picked image with enough of itself left to be told
      // apart: a large mark checked at a tenth of its size is a 25-pixel
      // blob, and five tries at a blob find one that fits -- measured, the
      // Kenvue K matched two small flags. The drawn words of the second
      // check keep the bars they were measured with.
      const at = origHit.scale;
      const copyW = template.width * at, copyH = template.height * at;
      const small = !ready.smallText && Math.min(copyW, copyH) < SMALL_COPY
        && copyW * copyH >= SMALL_COPY_AREA;
      const sizes = small ? [at, at * 0.985, at * 1.015, at * 0.97, at * 1.03] : [at];
      const hit = verify(gray, pageW, pageH, template, native, origHit,
        sizes.concat(item.scale * toOrig)) || origHit;
      if (hit.score > best) best = hit.score;
      if (hit.score >= threshold) kept.push(hit);
      // The hit, not just its score. A bar the reviewer set too high can then
      // be lowered without searching the document again — the answer is
      // already here, on the wrong side of a number.
      else turnedAway.push(hit);
    }
    // Nothing survived the gate, so the closest thing seen is refinement's own
    // best — mapped back to original coordinates.
    if (!kept.length) {
      for (const item of located) {
        const s = item.hit.score;
        if (s > best) best = s;
      }
    }
    ...
    // Only on modest pages. A photographed slide is already past the point
    // where another 1.5x helps letterforms, and it multiplies the cost of an
    // already-expensive sweep.
    const upscale = ready.upscaled ? 1 : upscaleFor(ready, assets, opts);
    if (upscale > 1) {
      const up = upscaledOf(gray, pageW, pageH, upscale);
      // `upscaled` stops the second pass asking for a third.
      const found = searchPage(up.gray, up.width, up.height,
        { ...ready, smallText: false, upscaled: true }, opts);
      const factor = 1 / upscale;
      if (found.best > bestScore) bestScore = found.best;
      for (const hit of found.near || []) {
        turnedAway.push({ ...hit,
    ...
    return { matches: Match.suppress(matches, opts.overlap), best: bestScore,
             near: Match.suppress(turnedAway, opts.overlap) };
  }
  ...
  async function searchAll(pages, entries, options, onProgress) {
    ...
    for (const entry of entries) {
      ...
      pageIndexes: entry.pageIndexes || null, seeds: entry.seeds || null, ...prepared,
    });
    }
    ...
    for (const page of pages) {
      ...
      // How coarse this page is, which only the page knows. Cloned once per
      // page rather than per template: the options are otherwise shared.
      const dpi = dpiOf(page);
      const pageOpts = dpi ? { ...opts, pageDpi: dpi } : opts;

      for (const item of ready) {
        // Optional per-template page list: the second check only correlates where
        // OCR/text did not already settle the term.
        if (item.pageIndexes && !item.pageIndexes.has(page.index)) continue;
        const found = searchPage(gray, pageW, pageH, item, seedOpts(pageOpts, item, page.index));
        const into = results.get(item.key);
        if (found.best > into.best) into.best = found.best;
        for (const hit of found.near || []) into.near.push({ ...hit, pageIndex: page.index });
  ...
  // The seeds that belong to this page, handed to searchPage as its own.
  // Held on the entry rather than the options because they are about one
  // term: two terms searched over the same page have different ones.
  function seedOpts(opts, item, pageIndex) {
    if (!item.seeds || !item.seeds.length) return opts;
    const mine = item.seeds.filter(seed => seed.pageIndex === pageIndex);
    return mine.length ? { ...opts, seeds: mine } : opts;
  }
  ...
  function searchAllParallel(pages, entries, options, onProgress) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let watch = null;
      const finish = () => {
        if (watch !== null) { clearInterval(watch); watch = null; }
      ...
      // Asked for often, not once a page. A page of a large document is
      // twenty to forty seconds of work, and Stop that waits for the page in
      // flight is Stop that does nothing for half a minute — long enough that
      // the reviewer presses it again, or decides it is broken. Terminating
      // the workers ends the run where it stands; the pages already answered
      // are kept, and `stoppedAfter` says how many that was, which is what a
      // stopped run has always reported.
      const halt = () => {
      ...
      if (stopper) watch = setInterval(() => { if (shouldStop()) halt(); }, 120);
      ...
      const feed = worker => {
        ...
        // How coarse the page is travels with it: the worker has the pixels
        // but not the paper size they were rendered from.
        const dpi = dpiOf(page);
        if (!canBitmap) {
          const gray = grayFor(page);
          worker.postMessage({ type: 'page', index: page.index, width, height, dpi, gray },
            [gray.buffer]);   // transferred, so nothing is copied
          return;
        }
        createImageBitmap(page.source).then(bitmap => {
          if (stopped) { bitmap.close(); return; }
          worker.postMessage({ type: 'page', index: page.index, width, height, dpi, bitmap },
            [bitmap]);
        }).catch(() => {
      ...
      for (const worker of workers) {
        ...
        worker.onmessage = event => {
          ...
          for (const found of message.results) {
            ...
            if (!into) continue;
            if (found.best > into.best) into.best = found.best;
            for (const hit of found.near || []) into.near.push({ ...hit, pageIndex: message.index });
        ...
        worker.postMessage({
          ...
          entries: entries.map(entry => ({
            ...
            // Arrays clone; Sets do not survive postMessage.
            pageIndexes: entry.pageIndexes ? Array.from(entry.pageIndexes) : null,
  ...
  root.BlindedImageSearch = {
    ...
    cloneableOptions,
    upscaledOf, upscaleFor, dpiOf,
    SMALL_TEXT_UPSCALE, COARSE_PAGE_UPSCALE, COARSE_DPI, TINY_TEMPLATE_SHORT,
