...
(function (root) {
  ...
  function rereadCrops(items, toOrig, width, height) {
    const unsure = items.filter(it => it.rect
      ...
      && /[A-Za-z]/.test(it.str || '')
      && it.rect.h / toOrig <= REREAD_MAX_H && it.rect.h / toOrig >= 4)
      .map(it => ({ x: it.rect.x / toOrig, y: it.rect.y / toOrig,
        w: it.rect.w / toOrig, h: it.rect.h / toOrig, n: 1 }))
    ...
    for (const b of unsure) {
      const line = boxes.find(o => {
        ...
        return sameLine && gap < Math.max(o.h, b.h) * 3;
      });
  ...
  function enlarged(canvas, box) {
    const w = Math.round(box.w * REREAD_SCALE);
    const h = Math.round(box.h * REREAD_SCALE);
  ...
  function fromEnlarged(found, box, toOrig) {
    const k = toOrig / REREAD_SCALE;
    const at = (v, o) => (o + v / REREAD_SCALE) * toOrig;
  ...
  function rereadReplaces(word, items) {
    if (!word || !word.rect || !/[A-Za-z]{2,}/.test(word.str || '')) return null;
    ...
    if (!under.length) return word.confidence >= INVERT_MIN_CONF ? [] : null;
    const best = Math.max(...under.map(it => typeof it.confidence === 'number' ? it.confidence : 100));
    return word.confidence >= best + REREAD_GAIN ? under : null;
  }
  ...
  async function rereadUnsure(engineWorker, prepared, items, toOrig) {
    ...
    for (const box of boxes) {
      ...
      for (const word of found) {
        ...
        // a new reading's boxes are not all the same height: "Fraser and
        // Neave" re-read came back with "Fraser" a few pixels taller, sorted
        // after the other two, and the page's text said "and Neave Fraser".
  ...
  async function readWith(engineWorker, canvas) {
    ...
    const settled = PagePrep ? await rereadUnsure(engineWorker, prepared, items, toOrig) : items;
    settled.sort((a, b) => (a.lineY ?? a.y) - (b.lineY ?? b.y) || a.x - b.x);
  ...
  root.BlindedOcr = {
    ...
    rereadCrops, rereadReplaces, fromEnlarged, REREAD_SCALE,
  };
