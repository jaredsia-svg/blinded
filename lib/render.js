...
(function (root) {
  ...
  function flatten(source, boxes, texts, inks) {
    ...
    // covered it.
    // Anything drawn by hand sits with the writing, over the bars: a circle
    // round a covered figure means the covered figure, so it has to be on top
    // of the bar rather than under it.
    drawInks(ctx, inks || []);
    drawTexts(ctx, texts || []);
    return canvas;
  }

  // ---------- lines the reviewer drew by hand ----------
  //
  // Here for the same reason drawTexts is: the page on screen and the exported
  // page are painted by one function, so a stroke cannot be one shape in the
  // preview and another in the file.
  //
  // Points are in the source canvas's pixels and the width is too, so a stroke
  // keeps its thickness relative to the page however the page is displayed.
  function drawInks(ctx, inks) {
    for (const ink of inks) {
      const points = (ink && ink.points) || [];
      if (points.length < 1) continue;
      ctx.save();
      ctx.strokeStyle = ink.colour || '#000000';
      ctx.lineWidth = Math.max(1, ink.width || 1);
      ctx.lineCap = 'round';
      ctx.lineJoin = 'round';
      // A tap, rather than a drag: a dot of the same weight as the line would
      // have been. Without this a careful single press leaves nothing at all,
      // which reads as the tool being broken.
      if (points.length === 1) {
        ctx.fillStyle = ink.colour || '#000000';
        ctx.beginPath();
      ...
      // Through the midpoints, with each recorded point as the control: a
      // polyline through raw pointer samples is visibly faceted at the speed a
      // hand actually moves.
  ...
  // The box a stroke occupies, for putting its controls beside it and for
  // deciding whether a press landed on it.
  function inkBox(ink) {
    const points = (ink && ink.points) || [];
    if (!points.length) return { x: 0, y: 0, w: 0, h: 0 };
    let left = points[0].x, right = points[0].x;
    let top = points[0].y, bottom = points[0].y;
    ...
    const pad = Math.max(1, (ink.width || 1) / 2);
    return { x: left - pad, y: top - pad,
             w: (right - left) + pad * 2, h: (bottom - top) + pad * 2 };
  }
  ...
  root.BlindedRender = { flatten, drawLabel, legendCanvas, encodeForPdf, canvasToBlob, deflate,
    drawTexts, wrapText, textBox, drawInks, inkBox,
    TEXT_FONT, TEXT_LINE, MIN_LABEL_PX };
